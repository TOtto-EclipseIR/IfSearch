...
void IfSearch::processFace(void)
{
    ...
    QPair<QString,DetectorResult> face = pendingFaces.takeFirst();
    QString imageId = face.first;
    DetectorResult result = face.second;
    ...
    if (fwpGenerate->isActive() && optGenerateEnable->toBool())
    {
        ...
        if (roiColor.isValid() || allColor.isValid())
        {
            ...
            ImageMarker marker(&generateImage);
            marker.rect(headBox, roiColor, 3);
            marker.eyes(expectedEyes, roiColor, 0, 4);
            marker.rect(QQRect(expectedEyeSize,
                                    eyeCenters.left()),
                        roiColor);
            marker.rect(QQRect(expectedEyeSize,
                                    eyeCenters.right()),
                        roiColor);
            marker.rect(QQRect(QSize(minExpectedEyeWidth,
                                          maxExpectedEyeWidth),
                                    expectedEyes.left()),
                        roiColor);
            marker.rect(QQRect(QSize(minExpectedEyeWidth,
    ...
    if (rtn.is(EigenFace::ReturnNoHead))
    {
...
//            QColor roiColor = optMarkEyeRoiColor->value<QColor>();
            if (noEyesColor.isValid())
            {
                ...
                QQRect overRect(head);
                QQRect overMark(head);
    ...
    else
    {
        ...
        if (optClothesEnable->toBool())
        {
            ...
            if (clothesMatcher->isMatch() || fwpNoClothes->isActive())
            {
                ...
                ImageMarker marker(&clothesImage);
                marker.rect(head, optMarkFaceColor->value<QColor>(), 3);
                marker.end();
        ...
        if (optMatchEnable->toBool() && ! eigenFace->templat().isEmpty())
        {
            ...
            EigenFaceSearchResultList resList
                    = eigenMatcher->search(EigenFaceSearchSettings::CasualMatch,
                                           eigenFace->templat(),
                                           &matchSettings);
            ++numSearches;
