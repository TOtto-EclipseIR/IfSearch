...
WindowsProcess::WindowsProcess(QObject * parent)
	: QObject(parent)
{
	Return::add(ReturnStatusNull, "DDTcore Null Status", Warning);
	Return::add(ReturnMemory, "%1 Memory: %2 Total, %3 Available (%4%)", Detail);
	Return::add(ReturnLoadFail, "Library Load Failed for %1", Warning);
}
