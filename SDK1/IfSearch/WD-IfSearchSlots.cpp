...
void IfSearch::changed(const QString & key)
{
...
#ifdef ENABLE_WATCHDOG
    else if (key == optWatchDogValue->keyName())
    {
        emit petDog(*optWatchDogValue);
    }
#endif
    else if (0 == key.compare(tr("Input/URL", "config"), Qt::CaseInsensitive))
    {
        ...
        else if (0 == url.scheme().compare("dir", Qt::CaseInsensitive))
        {
            ...
            if (err.isEmpty())
            {
                ...
                ERROR(tr("Error starting %2: %1", "error message"),
        ...
        else if ("http" == url.scheme())
        {
            ...
            appSettings->objectProperties(camera, tr("Input", "config"), qsl, DDT::Settings::Volatile);
            qsl.clear();
            qsl << "MaxCache";
            appSettings->objectProperties(camera, tr("Input", "config"), qsl, DDT::Settings::Volatile | DDT::Settings::Advanced);
            ...
            if ( ! err.isEmpty())
                ERROR(tr("Error starting %2: %1", "error message"), err, optInput->toString());
            else
            {
                camera->start();
                changed(optPause->keyName());
    ...
    else if (0 == key.compare(tr("Enroll/Command", "config"), Qt::CaseInsensitive))
    {
        ...
        {
            ERROR("[ENROLL] Command mode busy in %1=%2", commandMode,
        ...
        else
        {
            if ( ! optEnrollCommand->toString().isEmpty())
                ERROR("[ENROLL] Bad Command %1", optEnrollCommand->toString());
            commandMode.clear();
        }
    }
    else if (0 == key.compare(tr("Search/Command", "config"), Qt::CaseInsensitive))
    {
        if ( ! commandMode.isEmpty())
        {
            ERROR("[SEARCH] Command mode busy in %1=%2", commandMode,
                  appSettings->value(commandMode+"/Command").toString());
        ...
        else
        {
            if ( ! optSearchCommand->toString().isEmpty())
                ERROR("[SEARCH] Bad Command %1", optSearchCommand->toString());
            commandMode.clear();
        }
    }
    else if (0 == key.compare(optRetrieveCommand->keyName(), Qt::CaseInsensitive))
    {
        if ( ! commandMode.isEmpty())
        {
            ERROR("[RETRIEVE] Command mode busy in %1=%2", commandMode,
                  appSettings->value(commandMode+"/Command").toString());
        ...
        else
        {
            if ( ! optRetrieveCommand->toString().isEmpty())
                ERROR("[RETRIEVE] Bad Command %1", optRetrieveCommand->toString());
            commandMode.clear();
        }
    }
    ...
    else if (0 == key.startsWith("Search/", Qt::CaseInsensitive))
        searchSettings.update();
    else if (0 == key.startsWith("Match/", Qt::CaseInsensitive))
        matchSettings.update();
