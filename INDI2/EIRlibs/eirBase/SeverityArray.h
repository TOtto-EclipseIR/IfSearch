/*! \file SeverityArray.h Declarations for SeverityArray template
  *
  */
...
#if 1
#include "EnumerationArray.h"

template <class T> class SeverityArray
        : public EnumerationArray<Severity, T>
{
public:
    SeverityArray(void)
    { EnumerationArray<Severity, T>::fill(); }
    SeverityArray(const T & t)
    { EnumerationArray<Severity, T>::fill(t); }
};
