...
void Return::staticCtor(void)
{
	...
	add(QFileErrorCode, QObject::tr("Error #%1 (%2) on file: %3"), Error);
	add(QFileWarningCode, QObject::tr("Error #%1 (%2) on file: %3"), Warning);
	add(NotImplemented, QObject::tr("Not Implemented: %1"), Error);
	add(NullPointer, QObject::tr("Null Pointer: %1"), Fatal);
	add(CantOpenFile, QObject::tr("Can't open file %1 for %2"), Error);
	add(ReturnXmlParse, QObject::tr("Error parsing xml in %1: %2 at %3,%4"), Error);
} // staticCtor()
...
QVariant Return::operator[](int x) const
{
    if (x > 0 && x <=4)
        return  var[x-1];
    else
