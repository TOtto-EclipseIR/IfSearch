...
QImage ImageCache::getImage(const QString &imageId, QSize szDesired)
{
    ...
    if (ice->orig.isNull())
    {
        ...
        ice->curBytes += ice->orig.byteCount();
        curBytes +=  ice->orig.byteCount();
    }
                         ...
                         const QString & cameraId)
{
    ...
    ice->orig = img;
    ice->curBytes = ice->baFileData.size() + ice->orig.byteCount();
    curBytes += ice->curBytes;
    grabPending.append(ice->imageId);
    use(ice->imageId);
...
QString ImageCache::grab(const QByteArray & ba,
                         ...
                         const qint64 epochMsec)
{
    QImage img = QImage::fromData(ba, format.toLatin1());
    if (img.isNull())
        return QString();
