...
Return EigenFaceBitMask::readXmlFile(const QString & fileName)
{
	QString errMsg;
	int errLine, errCol;
	QDomDocument doc;
...
Return EigenFaceBitMask::read(const QDomElement & de)
{
...
Return EigenFaceBitMask::read(const QByteArray & ba)
{
	QBuffer buf;
	buf.setData(ba);
	return read(&buf);
...
Return EigenFaceBitMask::read(QIODevice * io)
{
	int i;
	...
	if (StreamId != i)
		return Return(EigenFace::ReturnInvalidStructure, "EFBM:StreamId");
...
Return EigenFaceBitMask::write(QIODevice * io) const
{
	if ( ! isValid())
		return Return(EigenFace::ReturnInvalidStructure, "Bitmask");
...
Return EigenFaceBitMask::write(QByteArray * ba) const
{
	Return rtn;
...
Return EigenFaceBitMask::write(QDomElement * de) const
{
	if ( ! isValid())
		return Return(EigenFace::ReturnInvalidStructure, "BitMask");
...
Return EigenFaceBitMask::write(QDomDocument * doc) const
{
	if ( ! isValid())
		return Return(EigenFace::ReturnInvalidStructure, "BitMask");
