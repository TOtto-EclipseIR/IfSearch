...
//DDT::Return EigenFaceData::staticReturn;
...
DDT::Return EigenFaceData::checkRootAttributes(const QDomElement & de)
{
    ...
    if ( ! Class.isEmpty() && ! EigenClass.isEmpty() && Class != EigenClass)
        return DDT::Return(EigenFace::ReturnConflict, "Class", Class, EigenClass);
    if (Pixels && ActivePixels && Pixels != ActivePixels)
        return DDT::Return(EigenFace::ReturnConflict, "ActivePixels", ActivePixels, Pixels);
    if (Layers && ActiveLayers && Layers != ActiveLayers)
        return DDT::Return(EigenFace::ReturnConflict, "ActiveLayers", ActiveLayers, Layers);
    if ( ! newSize.isEmpty() && ! EigenSize.isEmpty() && newSize != EigenSize)
        return DDT::Return(EigenFace::ReturnConflict, "EigenSize", EigenSize, newSize);
    if ( ! newLine.isNull() && ! EigenEyeLine.isNull() && newLine != EigenEyeLine)
        return DDT::Return(EigenFace::ReturnConflict, "EigenEyeLine", EigenEyeLine, newLine);
...
DDT::Return EigenFaceData::loadFrom(QFile & xmlFile)
{
    DDT::Return rtn;
    QString errMsg;
    int errLine, errCol;
    ...
    if ( ! xmlFile.open(QIODevice::ReadOnly))
        rtn = DDT::Return::qfileError(xmlFile);
    else if ( ! doc.setContent(&xmlFile, &errMsg, &errLine, &errCol))
        rtn = DDT::Return(DDT::Return::ReturnXmlParse,
        ...
        else if (bitMaskTagName == de.tagName())
        {
            ...
            if ( ! rtn.isError())
            {
                if (EigenSize != Mask.size())
                    rtn = DDT::Return(EigenFace::ReturnConflict,
...
DDT::Return EigenFaceData::create(const QDir & initDir)
{
    DDT::Return rtn;
