...
bool IfSearch::avgFaceFinish(void)
{
    ...
    QImage eigenFace = INDI::EigenFace::normalize(avgImage,
                                            eigenParms->normalEyes(),
                                            eigenData->eigenSize(),
                                            eigenData->eigenEyes(),
                                            QImage::Format_Indexed8);
    INDI::EigenFaceMaskedArray avgArray(eigenData);
    ...
    QDomElement de = doc.createElement(INDI::EigenFaceData::meanFaceTagName);
    avgArray.write(&de);
    root.appendChild(de);
