...
#if 0 //ndef IJM
DDT::VersionInfo DDTcore::version(VERSION_MAJOR, VERSION_MINOR, VERSION_MAINT, VERSION_BUILD,
...
Return DDTcore::read(QDomDocument * doc, QFile * file, const QString & docType)
{
    ...
    if ( ! doc->setContent(file, &errMsg, &errLine, &errCol))
        return Return(DDT::Return::ReturnXmlParse, file->fileName(), errMsg, errLine, errCol);
...
DDT::Return DDTcore::writeEmptyFile(QString fileName)
{
    if (fileName.isEmpty())
        return DDT::Return::errorString("DDTcore::writeEmptyFile() no file name");

    QDir dir(QFileInfo(fileName).dir());
    if ( ! dir.exists())
    {
        if ( ! dir.mkpath("."))
            return DDT::Return::errorString(QObject::tr("DDTcore::writeEmptyFile() "
                                "unable to make path %1").arg(dir.path()));
    }

    QFile f(fileName);
    if ( ! f.open(QIODevice::WriteOnly | QIODevice::Truncate))
        return DDT::Return::qfileError(f);
    f.close();
