...
void IfSearch::doDelete(void)
{
    ...
    DDT::Return rtn = faceBase->deleteFace(faceKey);
    if (rtn.isError())
...
void IfSearch::doRemove(void)
{
    ...
    DDT::Return rtn = faceBase->removePerson(enrollPersonKey);
    if (rtn.isError())
        return enrollError(rtn.toString());
...
void IfSearch::doRemoveAll(void)
{
    ...
    if (0 == enrollPersonId.compare("Confirm", Qt::CaseInsensitive))
    {
        ...
        DDT::Return rtn = faceBase->removeAll();
        if (rtn.isError())
            return enrollError(rtn.toString());
    ...
    else if (0 == enrollPersonId.compare("Destroy", Qt::CaseInsensitive))
    {
        ...
        DDT::Return rtn = faceBase->destroy();
        if (rtn.isError())
            return enrollError(rtn.toString());
        enrollResults = tr("RemovedAll");
    }
...
void IfSearch::doRename(void)
{
    ...
    DDT::Return rtn = faceBase->renamePerson(enrollPersonKey, newPersonId);
    if (rtn.isError())
        return enrollError(rtn.toString());
...
void IfSearch::doReload(void)
{
    ...
    DDT::Return rtn = faceBase->initialize(faceBaseBaseDir, faceBaseMaxLoad);
    RETURN(rtn);
...
void IfSearch::doManual(void)
{
        ...
        else
        {
            if ( ! enrollPersonId.isEmpty())
            {
                ...
                if ( ! enrollPersonKey)
                {
                    ...
                    DDT::Return rtn = faceBase->add(&person);
                    if (rtn.isError())
    ...
    eigenFace->clear();
    DDT::Return rtn = eigenFace->setImage(manualImage);
    RETURN(rtn);
    ...
    if (rtn.isError())
        return enrollError(rtn.toString());
    INDI::EigenFaceTemplate tpl(eigenFace->templat());
    if (tpl.isEmpty())
    ...
    face.setStatus(INDI::EigenFaceStatus::Active);
    face.setFaceKey(0);
