...
DDT::Return EigenFaceVector::read(const QDomElement & de)
{
	DDT::Return rtn;
	...
	if (n != coefs.size())
		return DDT::Return(EigenFace::ReturnInvalidStructure, "Bad Count");
...
DDT::Return EigenFaceVector::write(QDomElement * de) const
{
	if ( ! isValid())
...
DDT::Return EigenFaceVector::write(QByteArray * ba) const
{
	DDT::Return rtn;
...
DDT::Return EigenFaceVector::write(QIODevice * io) const
{
	if ( ! isValid())
		return DDT::Return(EigenFace::ReturnInvalidStructure, "Vector");
...
DDT::Return EigenFaceVector::read(const QByteArray & ba)
{
	QBuffer buf;
	buf.setData(ba);
	return read(&buf);
...
DDT::Return EigenFaceVector::read(QIODevice * io)
{
	int i;
	...
	if (StreamId != i)
		return DDT::Return(EigenFace::ReturnInvalidStructure, "EFV:StreamId");
