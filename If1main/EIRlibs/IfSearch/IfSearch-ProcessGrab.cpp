...
{
    FUNCTION();
    QList<INDI::DetectorResult> resultList;
    ...
    idGenerator.setBaseMsd(streamBase_mst.delta(grabTime));
    ...
    if (detectEnabled)
    {
        ...
        {
            ERROR("Error in Face Detector for %1", imageId);
            resultList.clear();
        }
        else
        {
    ...
    else
    {
        ...
        {
            QRect head = DDT::QQRect(image.size() * inputOverCrop,
                               image.rect().center());
            INDI::DetectorResult result(1, 0, head);
    ...
    else
    {
        ...
        {
            INDI::DetectorResult result = resultList.takeLast();
            pendingFaces.append(QPair<QString,INDI::DetectorResult>(imageId, result));
        ...
        pendingFaces.append(QPair<QString,INDI::DetectorResult>(imageId, INDI::DetectorResult()));
    }
