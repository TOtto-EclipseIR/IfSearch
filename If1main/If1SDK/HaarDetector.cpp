...
DDT::Return HaarDetector::initialize(QFile * xmlFile)
{
    QString errMsg;
    int errLine, errCol;
...
void HaarDetector::setImage(QImage img)
{
        ...
        unsigned w = imgOrig.width(), h = imgOrig.height() & 0xFFFC;
        if (w <= 0 && h <= 0)
        {
            if (ipl_Grey)
                cvReleaseImage(&ipl_Grey);
...
QImage HaarDetector::image(void) const
{
    ...
    case C:
        ...
        if (ipl_Grey && 1 == ipl_Grey->nChannels)
        {
            ...
            result.setColorTable(DDT::GreyImage::greyColorTable());
...
bool HaarDetector::loadXmlCascade(const QString & xmlFilename)
{
    ...
    if ( ! hasDetector())
    {
        ERROR("Unable to load cascade at %1", DetectorFileName);
...
QImage HaarDetector::detectImage(void) const
{
    ...
    DDT::ImageMarker marker(&resultImage);
