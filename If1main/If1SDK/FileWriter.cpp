...
/* USAGE: (.h)
	DDT::StatusHandler  sts;
	DDT::ImageCache cache;
...
(.cpp)
	appSettings = new DDT::Settings(...);
	writer = new DDT::FileWriter(appSettings, QString(), this);
	writer->setImageCache(&cache);
	writer->addLogFile(&sts)
	fwpCapture = writer->newProfile("Capture", DDT::FileWriter::CaptureImage);
	fwpFaceCache = writer->newProfile("FaceCache", DDT::FileWriter::Cache | DDT::FileWriter::FaceImage);
	...
    ...
    : QObject(parent), _settings(settings)
{
    ...
    Return::add(ReturnNoCache, "No Image Cache for %1", DDT::Warning);
    Return::add(ReturnNullImage, "Null Image for %2: %1", DDT::Warning);
}
...
void FileWriter::setImageCache(DDT::ImageCache * cache)
{
    FUNCTION();
    _imageCache = cache;
} // setImageCache()
...
//    FUNCTION();
    ...
    if (maxCache)
        foreach (FileWriteProfile * profile, _mapKeyToProfile)
            if (Cache & profile->_flags)
                foreach (QDir dir, profile->_dirs)
                {
        QStringList fileNames = dir.entryList(QDir::Files, QDir::Time);
        TRACE("cacheClean() %1 has %2 files", profile->_name, fileNames.size());
