...
void IfSearch::enrollGrab(void)
{
    ...
    DDT::Return rtn = tpl.read(fi.absoluteFilePath());
    if (rtn.isError())
    ...
    RETURN(rtn);
    enrollResults += resultsDelimiter + tr("%1 enrolled as %2:%3 %4").arg(imageId).arg(face.personKey())
                     .arg(face.faceKey()).arg(face.reason().isEmpty() ? "Active" : face.reason());
