...
namespace DDT
{
	...
	public:
		...
		bool isWarn(void) const { return severity >= DDT::Warning; }
		bool isError(void) const { return severity >= DDT::Error; }
		bool isFatal(void) const { return severity >= DDT::Fatal; }
		int returnCode(void) const { return code; }
		QString toString(void) const;
		QVariant operator[](int x) const { return (x > 0 && x <=4) ? var[x-1] : QVariant(); }
	...
	public: // static functions
		static void add(const int code, const QString & msg, const DDT::Severity sev);
#ifdef WINDOWS
	...
	private: // static variables
		...
		static QMap<int, DDT::Severity>	severities;
	}; // class Return
} // DDT
