...
bool HaarDetector::isStrange(DDT::QQRect rect)
{
    bool rtn = false;
...
void HaarDetector::groupByOverlap(bool returnAll)
{
    ...
    QMultiMap<int, DDT::QQRect> sortByArea;
    ...
    foreach (DDT::QQRect rect, allObjects)
        if ( ! isStrange(rect))
    ...
    allResults.clear();
    QMutableMapIterator<int, DDT::QQRect> it(sortByArea);
    it.toBack();
    while (it.hasPrevious())
    {
        // 3a. Take largest remaining to start next result
        it.previous();
        DDT::QQRect rect1 = it.value();
        it.remove();
...
void HaarDetector::groupByNeighbors(bool returnAll)
{
    ...
    while (it.hasPrevious())
    {
        ...
        while (it.hasPrevious())
        {
            ...
            it.previous();
            DDT::QQRect rect2 = it.value();
            if (DDT::QQRect(ri.rect).isNeighbor(rect2, fNeighbor))
            {
                it.remove();
...
void HaarDetector::groupByCenters(bool returnAll)
{
    ...
    while (itSort2.hasPrevious())
    {
        ...
        while ( ! list.isEmpty())
        {
            DDT::QQRect rect = list.takeFirst();
            int area = rect.area();
            int k = 1;
